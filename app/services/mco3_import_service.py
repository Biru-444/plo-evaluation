...
ถ้าแก้ : กฎการ flag (category_mismatch/checkbox_ambiguous/duplicate_course_code/
         curriculum_mismatch) อยู่ใน SYSTEM_INSTRUCTION ด้านล่างล้วนๆ ห้ามให้โมเดลเดาเอง — ถ้าจะ
...
ตาม schema ที่กำหนด อ่านเอกสาร PDF ที่แนบมาอย่างละเอียด แล้วกรอกข้อมูลตามนี้:
...
- clo_plo_mapping: คู่ (clo_code, plo_code) ที่เอกสารระบุชัดเจนว่า CLO ข้อไหนสัมพันธ์กับ PLO ข้อไหน
  ใส่เฉพาะคู่ที่มั่นใจจริงๆ เท่านั้น ห้ามเดา - ตารางความสัมพันธ์ PLO×CLO ในเอกสาร มคอ.3 มี 2 แบบ:
    ...
    (ข) ตารางแบบกาเครื่องหมาย (checkbox หรือช่องว่างให้ทำเครื่องหมาย ✓) ที่มีแถวเป็น CLO
        ...
        ไฟล์ต้นฉบับเทียบเองว่าช่องไหนถูกกาจริง
...
- flags: รายการปัญหา/ข้อสังเกตที่แอดมินต้องตรวจสอบเอง ก่อนบันทึกข้อมูลจริง (Phase นี้ยังไม่บันทึก
  ...
  checkbox_ambiguous ด้านบน ยังมีอีก 2 กรณีที่ต้อง flag:
    - "duplicate_course_code": ถ้ารหัสวิชาที่ปรากฏในเอกสารขัดแย้งกันเอง (เช่นหัวกระดาษหน้าแรกเขียน
      รหัสวิชาไม่ตรงกับที่ระบุในตารางแผนการสอน/หมวดอื่น) ให้ระบุในข้อความว่าเจอค่าไหนบ้างที่ตำแหน่งไหน
    ...
    - "other": ปัญหาอื่นที่ไม่เข้า 4 ประเภทข้างต้น แต่คิดว่าแอดมินควรรู้ก่อนบันทึกข้อมูล
