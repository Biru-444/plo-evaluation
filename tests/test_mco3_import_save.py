...
ด้วย AI" - ดู app/routes/course_import.py) บันทึก Course + CLO + CLOPLOMapping จริงจากผลลัพธ์ที่
...
def test_save_is_atomic_nothing_persists_when_clo_codes_collide_within_payload(
    ...
    payload = _payload(curriculum.id, course_code="TEST104")
    payload["clos"] = [
    ...
    # atomic จริง - course ที่ควรจะถูกสร้างก่อนหน้าต้องไม่ค้างอยู่ในฐานข้อมูล
